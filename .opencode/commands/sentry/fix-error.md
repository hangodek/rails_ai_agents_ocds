...
5. **Launch background agent**: Use the `task` tool routed to the `implementation-agent` (or `general`), in background/parallel mode if available, with `description: "Fix Sentry {short_id}"`
